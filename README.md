# 👋 Hola, soy Brian

💻 Estoy aprendiendo programación y desarrollo de software desde cero.

Mi enfoque es **aprender haciendo**: construyo proyectos, cometo errores, los entiendo y sigo mejorando.

---

## 🚀 Actualmente estoy aprendiendo

### 💻 Desarrollo

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)

### 🔧 Herramientas

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)

### 🤖 Inteligencia Artificial

![AI](https://img.shields.io/badge/Artificial%20Intelligence-412991?style=for-the-badge&logo=openai&logoColor=white)

Utilizo herramientas de Inteligencia Artificial como apoyo durante mi aprendizaje y desarrollo.

La utilizo para:

- 🧠 Comprender conceptos y código.
- 🐛 Encontrar y solucionar errores.
- 💡 Generar y analizar ideas.
- 🏗️ Planificar proyectos.
- ⚙️ Automatizar tareas.
- 📝 Documentar y mejorar proyectos.

Mi objetivo no es simplemente copiar código, sino **entender lo que estoy construyendo**.

---

## 📚 Mi recorrido

Estoy construyendo mi aprendizaje paso a paso, empezando por los fundamentos de programación y avanzando hacia el desarrollo de aplicaciones y sistemas.

Actualmente estoy trabajando con:

- 🐍 Python
- 🌐 HTML
- 🎨 CSS
- ⚙️ Flask
- 🔧 Git
- 🐙 GitHub
- 🤖 Inteligencia Artificial

### 🔜 Próximamente

- 🗄️ SQL
- ⚡ JavaScript
- 🔌 APIs
- 🗃️ Bases de datos
- 🚀 Desarrollo de aplicaciones

---

## 📂 Proyectos

### 📚 Aprendiendo Programación

Repositorio donde documento mi recorrido de aprendizaje, ejercicios y proyectos.

👉 [Ver repositorio](https://github.com/scarfiabrian/aprendiendo-programacion)

---

## 🎯 Mi objetivo

Seguir aprendiendo programación, construir proyectos cada vez más completos y desarrollar las habilidades necesarias para trabajar en el mundo del software y realizar proyectos freelance.

> 💡 **Aprender haciendo. Equivocarse, entender y volver a intentarlo.**

---

## 📊 GitHub

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=scarfiabrian&show_icons=true&theme=tokyonight&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=scarfiabrian&layout=compact&theme=tokyonight&hide_border=true)

---

### 🟢 Estado actual

**Aprendiendo y construyendo 🚀**